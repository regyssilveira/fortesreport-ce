...
uses
  ...
  TASources, TATransformations, TADrawerCanvas, frxPrinter;
...
type
 ...
 {$IFDEF Windows}
  TfrxPrinterDrawer = class(TScaledCanvasDrawer)
  public
    constructor Create(APrinter: TfrxCustomPrinter);
  end;
 {$ENDIF}
...
uses
  Forms, frxChartEditorLaz, Printers, TAPrint, FPCanvas,
  TATools, TAStyles, frxDsgnIntf, frxUtils, frxRes,
  TAChartUtils, LResources, Math;
...
{$IFDEF Windows}
constructor TfrxPrinterDrawer.Create(APrinter: TfrxCustomPrinter);
begin
  inherited Create(APrinter.Canvas, (Max(APrinter.DPI.X, APrinter.DPI.Y) / Screen.PixelsPerInch), []);
end;
{$ENDIF}
...
procedure TfrxChartView.ClearTrans;
var
  I, J: Integer;
  tr: TAxisTransform;
begin
...
procedure TfrxChartView.ClearSource;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      if (TChartSeries(FChart.Series[I]).Source is TBuiltinListChartSource) then
        Exit;
...
procedure TfrxChartView.ClearStripes;
var
  I: Integer;
begin
  for I := 0 to FChart.AxisList.Count - 1 do
...
procedure TfrxChartView.ClearAll;
begin
  ClearTrans;
  ClearSource;
  ClearStripes;
...
begin
  ...
  if Canvas is TfrxPrinterCanvas then
    FChart.Draw(TfrxPrinterDrawer.Create(frxPrinters.Printer), Rect(FX, FY, FX1, FY1))
  else
  {$ENDIF}
