...
{$ELSE}
  Windows, ScktComp,
{$ENDIF}
  SysUtils, Classes, frxServerUtils, frxNetUtils, frxGZip, frxMD5
{$IFDEF Delphi6}
, Variants
{$ENDIF};
...
begin
  ...
  repeat
    ...
    try
      ticks := GetTickCount;
      while (FActive and (not FBreaked)) or FThread.FSocket.Socket.Connected do
      begin
        PMessages;
...
begin
  ...
  begin
    ...
    begin
      ...
      if i > 0 then
      begin
        TempName := Copy(s1, 1, i - 1);
        s2 := Copy(s1, i + 1, Length(s1) - i);
        if TempName = 'dialog_redirect' then
        begin
          i := Pos(AnsiString('='), s2);
          if i > 0 then
          begin
            TempName := Copy(s2, 1, i - 1);
