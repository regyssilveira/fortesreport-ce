...
uses
  ...
  {$IFDEF FPC},LazFileUtils, LCLProc{$ENDIF}
...
{$ENDIF}
  ...
  public
    ...
    procedure AddItem(Item: TfrxXMLItem); virtual;
    procedure Clear;
...
procedure TfrxXMLItem.SetData(const AName, AText, AValue: String);
begin
  FName := AName;
  FText := AText;
  FValue := AValue;
...
//        if (curpos = 3) and (Pos(AnsiString('!--'), Name) = 1) then
        if (curpos = 3) and (Len >= 3) and (Name[1] = '!') and (Name[2] = '-') and (Name[3] = '-') then
{$ELSE}
