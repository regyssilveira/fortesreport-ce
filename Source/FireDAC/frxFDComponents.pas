{ --------------------------------------------------------------------------- }
{ FireDAC FastReport v 2022 enduser components                                }
{                                                                             }
...
uses
  Windows, Classes, SysUtils, DB, frxClass, frxCustomDB, frxXML,
  {$IFDEF ANYDAC}
    uADCompClient, uADStanOption, uADCompDataSet, uADStanParam, uADGUIxFormsfConnEdit
  {$ELSE}
    FireDAC.DatS, FireDAC.Comp.Client, FireDAC.Stan.Option, FireDAC.Comp.DataSet,
    FireDAC.Stan.Pool
  {$ENDIF}
, Variants
{$IFDEF QBUILDER}
...
{$IFDEF ANYDAC}
  ...
  TfrmFDGUIxFormsConnEdit = TfrmADGUIxFormsConnEdit;
  TFDConnection = TADConnection;
  TFDParams = TADParams;
  TFDParam = TADParam;
  TFDMacroDataType = TADMacroDataType;
...
{$ENDIF}
  ...
  public
    ...
    procedure Prepare; virtual; {added by fduenas}
    function OpenOrExecute: Boolean; virtual; {added by fduenas}
{$IFDEF QBUILDER}
...
{$ENDIF}
  ...
  public
    ...
    function OpenOrExecute: Boolean; virtual; {added by fduenas}
    function ExecFunc: Variant; virtual; {added by fduenas}
  published
  ...
  public
    ...
    function OpenOrExecute: Boolean; override; {added by fduenas}
    function ExecFunc: Variant; override; {added by fduenas}
...
{$ENDIF}
  ...
  {$IFDEF ANYDAC}
   uADStanDef, uADStanUtil, uADDAptManager
  {$ELSE}
  FireDAC.Stan.Param, FireDAC.Stan.Def, FireDAC.Stan.Util, FireDAC.Phys
  {$ENDIF}
  ;
...
begin
 if not SameText(FDatabase.DriverName, AValue) then
  ...
  begin
   ...
   if Result then begin
     ...
     Result := (Length(sVar) >= 5) and (sVar[1] = '{') and
                (sVar[Length(sVar)] = '}') and not({$IFDEF ANYDAC}ADInSet{$ELSE}FDInSet{$ENDIF}(sVar[2], ['0' .. '9']));
     if Result then begin
       ...
       if AMasterDetail and (ADataSet is TfrxFDQuery) then begin
         ...
         try
           ...
           for iFld := 0 to oMasterFields.Count - 1 do begin
              {$IFDEF ANYDAC}
               lSkip := {$IFDEF AnyDAC_NOLOCALE_META} ADCompareText {$ELSE} AnsiCompareText {$ENDIF}
              {$ELSE}
               lSkip := {$IFDEF FIREDAC_NOLOCALE_META} FDCompareText {$ELSE} AnsiCompareText {$ENDIF}
              {$ENDIF}
...
{-------------------------------------------------------------------------------}
function TfrxFDQuery.OpenOrExecute: Boolean;
begin
 UpdateParams;
...
var SaveUnidirectional: boolean;
begin

 if FQuery = nil then Exit;

 SaveUnidirectional := FQuery.FetchOptions.Unidirectional;
 FDatabase := AValue;
 ...
 FQuery.FetchOptions.Unidirectional := SaveUnidirectional;
end; { SetDatabase }
...
{-------------------------------------------------------------------------------}
function TfrxCustomStoredProc.ExecFunc: Variant;
begin
...
{-------------------------------------------------------------------------------}
function TfrxCustomStoredProc.OpenOrExecute: Boolean;
begin
 result := false;// Do nothing, code Should be added on inherited Classes;
end;
...
{-------------------------------------------------------------------------------}
function TfrxFDStoredProc.ExecFunc: Variant;
begin
 if FStoredProc = nil then Exit;
 UpdateParams;
 result := FStoredProc.ExecFunc;
...
{-------------------------------------------------------------------------------}
function TfrxFDStoredProc.OpenOrExecute: Boolean;
begin
 UpdateParams;
