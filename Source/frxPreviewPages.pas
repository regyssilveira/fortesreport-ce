...
uses
  ...
  frxClass, frxXML, frxPictureCache, frxBaseGraphicsTypes, Printers
{$IFDEF FPC}
...
type
  ...
  TfrxSignaturePages = class(TStringList)
  protected
    FActive: Boolean;
  public
    constructor Create;
    ...
    function IsContaine(Name: string): Boolean;
...
//    FMouseInView: Boolean;    { used in the ObjectOver to deteterminate mouseleave/mouseenter events }
    ...
    FCachedGraphicState: TfrxCachedGraphicType;
    procedure AfterLoad;
    ...
    function ProcessHyperlink(Hyperlink: TfrxHyperlink): Boolean;
    procedure ThumbnailsUpdated(Sender: TObject; UpdateState: TfrxThumbnailsUpdateState);
  protected
    FSignaturePages: TfrxSignaturePages;

    function GetCount: Integer; override;
    function GetPage(Index: Integer): TfrxReportPage; override;
    function GetPageSize(Index: Integer): TPoint; override;
    procedure UpdateCachedObject(CachedObject: TfrxComponent; IsLoading: Boolean = False);
    function GetPictureCacheOptions: TfrxPictureCacheOptions; override;
  public
    ...
    function AddGraphic(Graphic: TGraphic): Integer; override;
    ...
    procedure FireMouseLeave; override;
    ...
    function GetPicture(CacheType: TfrxCachedGraphicType; ImageIndex: Integer): IfrxCachedGraphic; override;
...
{$ENDIF}
    ...
    procedure AddFrom(Report: TfrxReport); overload; override;
    procedure AddFromPreviewPages(vPreviewPages: TfrxCustomPreviewPages); overload; override;
...
//  FXMLDoc.AutoIndent := True;
  ...
  if Assigned(AReport.PreviewPagesList) and Assigned(AReport.PreviewPagesList.Main) then
    FPictureCache.PictureCacheOptions.Assign(AReport.PreviewPagesList.Main.PictureCacheOptions);
  FPictureCache.OnThumbnailUpdated := ThumbnailsUpdated;
  FLastY := 0;
  FLastObjectOver := nil;
  FLastDrill := nil;
  FSignaturePages := TfrxSignaturePages.Create;
  FCachedGraphicState := cgOriginal;
end;
...
begin
  ...
  try
    ...
    if Assigned(Report.Preview) then
      Report.Preview.Unlock(False); // do not refresh, prevent flickering
  end;
end;
...
procedure TfrxPreviewPages.ThumbnailsUpdated(Sender: TObject;
  UpdateState: TfrxThumbnailsUpdateState);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if Supports(c, IfrxCachedView, LCachedView) then
            LCachedView.SetCachedGraphic(FPictureCache);
...
begin
  Picture.ImageIndex := FPictureCache.AddPicture(Picture.GetGraphic);
end;
...
procedure TfrxPreviewPages.UpdateCachedObject(CachedObject: TfrxComponent; IsLoading: Boolean);
var
  LCachedView: IfrxCachedView;
begin
  if Supports(CachedObject, IfrxCachedView, LCachedView) and LCachedView.IsUpdateRequired(FCachedGraphicState) then
    LCachedView.GetCachedGraphic(FCachedGraphicState, FPictureCache);
end;
...
begin
  ...
  FPictureCache.BuildFinished;
end;

procedure TfrxPreviewPages.FireMouseLeave;
var
...
begin
  ...
  EventParams.Scale := 1;
  EventParams.EditMode := dtSelect;
  if Assigned(FLastObjectOver) then
    Report.DoMouseLeave(FLastObjectOver, 0, 0, nil, EventParams);
  FLastObjectOver := nil;
  FLastDrill := nil;
  Report.Preview.Hint := '';
...
begin
  ...
  begin
    if xi[0].Prop['l'] <> '' then
      ...
      LeftX := 0;
    CorrX := X - LeftX;
    { special case for crosstabs #307606 , #549920 }
    { do not try to restore column position for cross-bands }
    { we have to remove it when crosstab will use Table }
    if (CompareText(xi[0].Name, 'TfrxNullBand') = 0) and (CorrX < 0) and (X = 0) then CorrX := 0;
...
{$IFDEF FRVIEWER}
      ...
      try
        ...
        begin
           ShellExecute(0, 'open', PChar(ExtractFilePath(Application.ExeName) + 'FastReportFMX_Viewer.exe'), PChar(FileName), '', SW_SHOW);
...
{ load the picturecache }
  FPictureCache.LoadFromXML(FXMLDoc.Root);
...
//  xi := FXMLDoc.Root.FindItem('picturecache');
  FPictureCache.SaveToXML(FXMLDoc.Root);
  xi := FXMLDoc.Root.FindItem('postprocessing');
  xi.Clear;
...
var
  ...
  begin
    ...
    begin
      ...
      if Supports(c, IfrxDataLinkObject, DataLink) then
      begin
        sLink := DataLink.GetLink(dlmOnLoading);
        if sLink <> '' then
...
begin
  ...
  try
    ...
    else if FSignaturePages.Active and FSignaturePages.IsContaine(xi.Name) then
      Exit
    else
    begin
...
function TfrxPreviewPages.GetPicture(CacheType: TfrxCachedGraphicType;
    ImageIndex: Integer): IfrxCachedGraphic;
begin
  Result := FPictureCache.GetCachedBitmap(CacheType, ImageIndex);
end;

function TfrxPreviewPages.GetPictureCacheOptions: TfrxPictureCacheOptions;
begin
  if Assigned(FPictureCache) then
    Result := FPictureCache.PictureCacheOptions
  else
    Result := inherited GetPictureCacheOptions;
end;
...
begin
  ...
  //if (c = nil) then Exit;
  Component.frComponentState := Component.frComponentState + [csFrxModifyObject];
  try
...
begin
  Self.AddFromPreviewPages(Report.PreviewPages);
end;

procedure TfrxPreviewPages.AddFromPreviewPages(vPreviewPages: TfrxCustomPreviewPages);
var
  ...
  var
    ...
    sText: String;
  begin
    for idx := 0 to vPreviewPages.Outline.Count - 1 do
    begin
      vPreviewPages.Outline.GetItem(idx, sText, nPage, nTop);
      CurPage := nPageOffset + nPage;
      Outline.AddItem(sText, nTop);
      vPreviewPages.Outline.LevelDown(idx);
      CopyOutline;
      vPreviewPages.Outline.LevelUp;
      Outline.LevelUp;
    end;
  end;
...
begin
  ...
  for i := 0 to vPreviewPages.Count - 1 do
  begin
    Page := vPreviewPages.Page[i];
    xi := TfrxXMLItem.Create;
  ...
  AnchorRootSource := TfrxPreviewPages(vPreviewPages).FXMLDoc.Root.FindItem('anchors');
  ...
  {copy outline items}
  vPreviewPages.Outline.LevelRoot;
  Outline.LevelRoot;
  CopyOutline;
  if vPreviewPages.Count <> 0 then
  begin
    vPreviewPages.CurPage := vPreviewPages.Count - 1;
    FLastY := vPreviewPages.GetLastY;
    vPreviewPages.CurPage := -1;
  end;
...
function TfrxPreviewPages.AddGraphic(Graphic: TGraphic): Integer;
begin
  Result := FPictureCache.AddPicture(Graphic);
end;
...
var
  ...
  begin
    ...
    Result := RectVisible(Canvas.Handle, r) or (Canvas is TMetafileCanvas) {$IFDEF LCLGTK2} or (Canvas is TPrinterCanvas){$ENDIF} ;
  end;
begin
  IsPrinting := Canvas is {$IFDEF LCLGTK2}TPrinterCanvas{$ELSE}TfrxPrinterCanvas{$ENDIF};
  if IsPrinting then
    FCachedGraphicState := cgOriginal
  else
    FCachedGraphicState := cgThumbnail;
  Page := GetPage(Index);
...
//        else
          TfrxView(c).InteractiveDraw(Canvas, ScaleX, ScaleY, OffsetX, OffsetY, bHighlightEditable and (ferAllowInPreview in c.Editable));
        c.IsPrinting := False;
      end;
    end;
  end;
...
{$ENDIF}
    ...
    begin
      ...
      if (dup in [dmVertical, dmHorizontal]) or ((dup = dmNone) and Printer.IsDuplexActive) then
      ...
      try
        ...
        ShowModal;
          ...
          begin
            ...
            Report.PrintOptions.Printer := frxPrinters.Printer.Name;
...
function TfrxPreviewPages.ProcessHyperlink(Hyperlink: TfrxHyperlink): Boolean;
var
  Rep: TfrxReport;
  RepPage: TfrxPage;
  SPreview: TfrxCustomPreview;
  ReportLoaded: Boolean;
  lHyperlink: TfrxHyperlink;
...
begin
  Result := True;
  lHyperlink := TfrxHyperlink.Create;
  lHyperlink.Assign(Hyperlink);
  try

    case lHyperlink.Kind of
      hkURL:
        if lHyperlink.Value <> '' then
{$IFDEF FPC}
          LCLIntf.OpenURL(Hyperlink.Value);
{$ELSE}
          ShellExecute(GetDesktopWindow, nil, PChar(lHyperlink.Value), nil, nil,
            sw_ShowNormal);
{$ENDIF}
      hkAnchor:
        if lHyperlink.Value <> '' then
          SetToAnchor(lHyperlink.Value);
      hkPageNumber:
        if lHyperlink.Value <> '' then
          TfrxPreview(Report.Preview).PageNo := StrToInt(lHyperlink.Value);
      hkDetailPage:
        if Report.Preview.HasTab(lHyperlink.Value) then
          Report.Preview.SwitchToTab(lHyperlink.Value)
        else if not lHyperlink.DetailIsEmpty then
        begin
          RepPage := Report.FindObject(lHyperlink.DetailPage) as TfrxPage;
          if RepPage <> nil then
          begin
            lHyperlink.AssignVariables(Report);
            FireMouseLeave;
            Report.Terminated := False;
            Report.PreviewPagesList.AddAsCurrent;
            SPreview := Report.Preview;
            Report.Preview.Lock;
            try
              SPreview.AddPreviewTab(Report, lHyperlink.Value,
                lHyperlink.TabCaption, False, lHyperlink.DetailPage);
      ...
      hkDetailReport:
        ...
        begin
          ...
          Rep.PictureCacheOptions.Assign(Report.PictureCacheOptions);
          ReportLoaded := False;
          ...
          if Assigned(Report.OnLoadDetailTemplate) then
            ReportLoaded := Report.OnLoadDetailTemplate(Rep,
              lHyperlink.DetailReport, lHyperlink);
          if not ReportLoaded then
            ReportLoaded := Rep.LoadFromFile(lHyperlink.DetailReport);

          if ReportLoaded then
          begin
            lHyperlink.AssignVariables(Rep);
            Rep.PrepareReport();
            Report.Preview.AddPreviewTab(Rep, lHyperlink.Value,
              lHyperlink.TabCaption);
            FLastObjectOver := nil;
            FLastDrill := nil;
          end;
        end;
...
//        end;
      ...
      if Report.Terminated or Filter.Terminated then
  ...
  begin
    ...
    if Filter.ShowProgress then
      ...
      if not Report.EngineOptions.EnableThreadSafe then
        begin
          if Assigned(Report.OnProgress) then
            Report.OnProgress(Report, ptExporting, Index + 1);
          Application.ProcessMessages;
        end;
    ...
    if Filter.DoStart then
      try
        ...
        if Filter.ShowProgress then
          ...
          if not Report.EngineOptions.EnableThreadSafe then
            begin
              if Assigned(Report.OnProgressStart) then
                Report.OnProgressStart(Report, ptExporting, 0);
              Application.ProcessMessages;
            end;        
...
{$ENDIF}
          ...
          on Exceprion: EExportTerminated do
        ...
        if Filter.ShowProgress then
          ...
          if not Report.EngineOptions.EnableThreadSafe then
            begin
              if Assigned(Report.OnProgressStop) then
                Report.OnProgressStop(Report, ptExporting, 0);
              Application.ProcessMessages;
            end;
        Filter.DoFinish;
...
begin
  ...
  Filter.Report := Report;
  Filter.DefaultIOTransport := nil;
  if Assigned(Filter.OnBeforeExport) then
    Filter.OnBeforeExport(Filter);

  if (doShowExportSettings in Filter.ShowDialogOptions) then
    if (Filter.ShowModal <> mrOk) then
      Exit;
...
var
  ...
  ModObjList: TList;
  sHint: String;
  bIsHint, bHandleMessage: Boolean;
...
begin
  ...
  if Assigned(FLastDrill) then
    drill := FLastDrill;
  if Assigned(EventParams.SelectionList) and (pbSelection in Report.PreviewOptions.Buttons) and not (Assigned(v) and (ssShift in Shift)) and (aEventParams.MouseEventType = meMouseDown) then
  begin
  ...
  begin
    ...
    begin
      ...
      if Report.Preview.HasVisibleTabs then
  ...
  begin
    { #543558 - using of Highlight should not work, because }
    { Highlight is a report run-time property, but someone used it }
    { for preview object highlight and it worked in FR4 }
    { so we add it just for backward compat, this will be removed in future }
    if (FLastObjectOver is TfrxCustomMemoView) and TfrxCustomMemoView(FLastObjectOver).Highlight.Active then
      TfrxCustomMemoView(FLastObjectOver).ApplyHighlight(TfrxCustomMemoView(FLastObjectOver).Highlight);
  ...
  { #631690 call SetProgressMessage only once after all callbacks processed }
  { SetProgressMessage may use ProcessMessages cycle and event may appear inside }
  ...
  begin
    ...
    Report.SetProgressMessage(sHint, bIsHint, bHandleMessage);
  ...
  UnlockPage;
  Report.RunDelayedCommands;
end;

{ TfrxSignaturePages }

procedure TfrxSignaturePages.Activate;
begin
  FActive := True;
  Clear;
end;

constructor TfrxSignaturePages.Create;
begin
  inherited Create;
  FActive := False;
end;

procedure TfrxSignaturePages.Deactivate;
begin
  FActive := False;
end;

function TfrxSignaturePages.IsContaine(Name: string): Boolean;
begin
