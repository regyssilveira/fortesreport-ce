...
uses
    ...
    {$IFNDEF Linux}
    {$ELSE}
      , LCLType, LCLIntf, LCLProc, Controls, Dialogs, LazHelper
    {$ENDIF};
...
type
    ...
    private
      ...
      public function GetGlyphIndices(text: WideString; glyphs: PWord; widths: PInteger; rtl: boolean): Integer;
      {$ENDIF}
      ...
      function GetPostScriptName: WideString;
      // Methods
    public
      ...
      procedure BuildGlyphIndexList(used_glyphs: TList; uniscribe: boolean;
        ...
        {$IFDEF Linux}; SkipComposite: Boolean = False; SkipSpace: Boolean = True{$ENDIF}
        );
      ...
      property PostScriptName: WideString read GetPostScriptName;
    end;
          ...
          GlyphWidth := ((self.horizontal_metrix_table.Item[idx].advanceWidth * $3e8) div self.font_header.unitsPerEm) - 2;
{
    ...
    procedure TrueTypeFont.BuildGlyphIndexList(used_glyphs: TList; uniscribe: boolean; decompose: boolean; collate: boolean; use_kerning: boolean {$IFDEF Linux}; SkipComposite: Boolean = False; SkipSpace: Boolean = True{$ENDIF} );
    var
        location: Cardinal;
        composed_idx: Word;
    ...
    begin
      {$IFDEF Linux} //TODO: rewrite interaction with fonts.
        if Indices <> nil then
          Indices.Clear
        else
      {$ENDIF}
        ...
        begin
            ...
            begin
              ...
              begin
                composed_idx := Word(composed_indexes[j]);
                new_key := Indices.IndexOf(Pointer(composed_idx));
                if (not collate or (decompose and (new_key = -1))) {$IFDEF Linux} and not SkipComposite{$ENDIF} then begin
                  Indices.Add( Pointer(composed_idx)); //add part of the symbol (a)
                  ...
                  GlyphWidths.Add(Pointer(GlyphWidth)) //add part of the symbol (a)
                end;
              end;
              if (not collate or (Indices.IndexOf(Pointer(idx)) = -1)) then
              begin
                Indices.Add( Pointer(idx)); //add symbol (b)
                  GlyphWidth := ((self.horizontal_metrix_table.Item[idx].advanceWidth * $3e8) div self.font_header.unitsPerEm);
//                if (use_kerning and ((i < (used_glyphs.Count - 1)) and (self.kerning_table <> nil))) then
//                begin
//                  new_key := Word(used_glyphs[i + 1]);
...
//                end;
            ...
            begin
              new_key := Indices.IndexOf(Pointer(idx));
              if (new_key = -1) {$IFDEF Linux} or (not SkipSpace){$ENDIF} then
              begin
                Indices.Add(Pointer(idx));
                if key <> $667 then
                  begin
...
//        FTextMetric.otmPanoseNumber.ArmStyle := self.windows_metrix.Panose[6];
        ...
        FTextMetric.otmrcFontBox := Rect(font_header.font_header.xMin,
                                         font_header.font_header.yMax,
                                         font_header.font_header.xMax,
                                         font_header.font_header.yMin);
    ...
    function TrueTypeFont.GetGlyphIndices(text: WideString; glyphs: PWord; widths: PInteger; rtl: boolean): Integer;
    var
        text_as_array: TIntegerList;
        ch: WideChar;
        I: Integer;
    begin
        ...
        self.BuildGlyphIndexList(text_as_array, false, false, false, false, true, false);

        for I := 0 to self.Indices.Count - 1 do
        begin
          glyphs^ := Word(self.Indices[I]);
          Inc(glyphs);
        end;

        for I := 0 to self.Indices.Count - 1 do
        begin
          widths^ := Integer(self.widths[I+self.widths.Count-self.Indices.Count]);
          Inc(widths);
        end;
...
function TrueTypeFont.GetPostScriptName: WideString;
begin
  Result := name_table.Item[NameID_PostscriptName];
end;
                ...
                try
                  self.ListOfTables.Add( parsed_table)
                    ...
                    on ex: Exception do
                      raise;
                    //{$IFNDEF Linux}
//                      MessageBox(0, PChar(ex.Message), PChar('Font format error'), MB_YesNo)
                    //{$ELSE}
                      //ShowMessage('Gag! Font format error')
                    //{$ENDIF}
                end
            end
            else begin
