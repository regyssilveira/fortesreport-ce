...
{$ELSE}
  LCLType, LCLIntf,
{$ENDIF}
  SysUtils, Classes, Graphics, frxClass, frxPictureGraphics, frxExportBaseDialog
{$IFDEF Delphi6}
  , Variants
{$ENDIF};
{$IFNDEF FPC}
...
type
  ...
  protected
    ...
    FPicFormatName: String;
    procedure Save;
    procedure SaveToStream(const aStream: TStream); virtual;
    procedure FinishExport; virtual;
    procedure InitPictureFormat(const FormatName: String);
  public
...
uses
  frxUtils, frxFileUtils, frxRes, frxIOTransportIntf, frxrcExports,
  {$IFNDEF FPC}
    GIF,
  {$ENDIF}
...
begin
  InitPictureFormat(FPicFormatName);
  CurPage := False;
  FCurrentPage := 0;
  FYOffset := 0;
...
procedure TfrxCustomImageExport.InitPictureFormat(const FormatName: String);
begin
  FGHelper := GetGraphicFormats.FindByName(FormatName, [gcSaveTo, gcConvert]);
  if not Assigned(FGHelper) then raise Exception.Create(Format('Picture format %s not found.', [FormatName]));
end;
...
begin
  ...
  try
    ...
    try
      if Stream = nil then
      begin
        if not IOTransport.DoFilterProcessStream(s, Self) then
          SaveToStream(s);
      end
      else
        SaveToStream(s);
...
begin
  try
    Image := FGHelper.ConvertFrom(FBitmap, pf24bit, FJPEGQuality);
    try
