...
var
  m: TfrxIEMatrix;
begin
  m := TfrxIEMatrix.Create(UseFileCache, Report.EngineOptions.TempDir, Report.PictureCacheOptions.CachedImagesBuildType);
...
begin
  ...
  begin
    ...
    if Assigned(OnGenerateSheetName) then
      Name        := OnGenerateSheetName(Page.Name, Index);
  end;
...
begin
    ...
    if FPictures then
      for i := 0 to FMatrix.GetObjectsCount - 1 do
        with FMatrix.GetObjectById(i) do
          if (Image <> nil) and (Image.Width <> 0) then
            img.Add(Pointer(i));
  ...
  try
    ...
    begin
      ...
      try
        if not IOTransport.DoFilterProcessStream(fs, Self) then
          cd.Flush(fs);
      finally
