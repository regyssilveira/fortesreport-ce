...
{$ENDIF}
  ...
  private
    ...
    function AddString(s: string; supHTML: Boolean; FontSize: Integer): Integer; 
    ...
    function CreateSheetName(PageName: String; Index: Integer): String;
  public
...
function TfrxXLSXExport.AddString(s: string; supHTML: Boolean; FontSize: Integer): Integer; 
var i: integer;
          ...
          begin
           ...
           {$ENDIF}
            ...
            begin
              ...
              xlsxtags := TagToXLSX();
              //if xlsxtags <> '' then
              SharStr := SharStr + Format('<rPr><sz val="%d"/>', [FontSize]) + xlsxtags + '</rPr>';
...
begin
  ...
  WriteStr(FWorkbook, Format('<sheet name="%s" sheetId="%d" r:id="rId%d"/>',
    [m.Name, m.Index, m.Index], SetFormat),{$IFDEF Delphi12}True{$ELSE}True{$ENDIF});
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if b then
              begin
                k := AddString(String(Utf8Encode(Obj.Memo.Text)), Obj.HTMLTags, Obj.Style.Font.Size);
...
begin
  ...
  begin
    ...
    m.Index := Index + 1;
    m.Name := CreateSheetName(Page.Name, m.Index);
...
function TfrxXLSXExport.CreateSheetName(PageName: String; Index: Integer): String;
begin
  if Assigned(OnGenerateSheetName) then
    Result := OnGenerateSheetName(PageName, Index)
  else
    Result := frxGet(9154) + ' ' + IntToStr(Index);
end;
...
begin
  ...
  begin
    ...
    m.LastRow := FMatrix.Height - 2;
    m.Index := 1;
    m.Name := CreateSheetName(frxGet(9154) + ' ' + IntToStr(m.Index), m.Index);
    AddSheet(m);
    FMatrix.Clear;
  end
