...
uses
  ...
  Variants, frxImageConverter, frxExportBaseDialog, frxExportHelpers
{$IFDEF DELPHI16}
, System.UITypes
{$ENDIF}
;
...
uses frxUtils, frxFileUtils, frxUnicodeUtils, frxRes, frxrcExports, Math,
  {$IFNDEF Linux}ShellAPI, {$ENDIF}frxGraphicUtils, frxExportHTMLDialog;
...
var
  ...
  dcol, drow: Integer;
  text, buff: String;
  s, s1, sb, si, su, st, ol, stoolbar: String;
  Vert, Horiz: String;
  obj: TfrxIEMObject;
  EStyle: TfrxIEMStyle;
...
//        SaveGraphicAs(FBackImage, s, PictureType);
  ...
  begin
    ...
    ol := ol + '</ul>';
    WriteExpLn('<table class = "blockTableOutline" >');//style="left: 0%; position: fixed;"
    WriteExpLn('<tr>');
    WriteExpLn('<td style="overflow-y:scroll;";>');
    WriteExpLn(ol);
    WriteExpLn('</td>');
    WriteExpLn('</tr>');
    WriteExpLn('</table>');
    WriteExpLn('<table class = "blockReport">');
    WriteExpLn('<td>');
  end;
  ol := '';
  ...
  { Write outline id }
    ...
    begin
      ...
      if Assigned(FOutlineTreeNode) and (FOutlineTreeNode.Dest <> 0) then
        PageH := FMatrix.GetPageBreak(FOutlineTreeNode.Dest - 1);
      if Obj <> nil then
        ol := GetIdOutlineHTML(Obj.Top, Obj.Height, FOutlineTreeNode, PageH);
    end;

    WriteExpLn('<tr' + ol + ' style="height:' + IntToStr(drow) + 'px">');
...
{$ENDIF}
          ...
          begin
            ...
            begin
              s := GetPicsFolder + 'img' + IntToStr(Obj.ImageIndex) + '.' + GetPicFileExtension(PictureType);
...
//                SaveGraphicAs(Obj.Image, s, PictureType);
              ...
              if FServer then
                ...
                s := '';
              s := ReverseSlash( s + GetPicsFolderRel + 'img' + IntToStr(Obj.ImageIndex) + '.' + GetPicFileExtension(PictureType));
            end;
  ...
  begin
    WriteExpLn('</td>');
    WriteExpLn('</table>');
    WriteExpLn(outlineJS);
  end;

  WriteExpLn(FHTMLDocumentEnd.Text);
end;

procedure TfrxHTMLExport.SetMultipage(const Value: Boolean);
begin
  FMultipage := Value;
...
begin
  ...
  begin
    ...
    FMatrix.OnGetURL := FOnProcessHyperLink;
    FMatrix.ParentDetailURL := Self.ParentDetailURL;
    ...
    if vsExport in v.Visibility then
      begin
        if (v is TfrxCustomMemoView) or (v is TfrxLineView) or
          ...
          FMatrix.AddObject(v);
        if IsPageBG(v) and FExportPictures and FBackground then
        begin
...
begin
  ...
  begin
    if FAbsLinks then
      ...
      Result := ChangeFileExt({$IFNDEF Linux}frxUnixPath2WinPath{$ENDIF}(FileName), '.files');
