...
{$IFDEF USE_INDY}
  IdSMTP, IdMessage,
  IdSSLOpenSSL, IdExplicitTLSClientServerBase, IdAttachmentFile, IdEMailAddress
{$ELSE}
  frxSMTP
{$ENDIF}
    ...
    try
      ...
      begin
        ...
        try
          ...
          begin
            if (FUseMAPI = MSOutlook) and (FExportFilter.DefaultPath <> '') then
              FullFileName := FExportFilter.DefaultPath + PathDelim + ExtractFileName(FExportFilter.FileName)
            else
...
{$IFDEF USE_INDY}
procedure AddRecipients(EmailAddressList: TIdEmailAddressList; RcptList: string);
var i,j, len: integer;
begin
  i := 1;
  j := 1;
  ...
  begin
    if RcptList[i] = ',' then
    begin
      s := Copy(RcptList, j, i - j);
      EmailAddressList.Add.Text := s;
      j := i + 1;
    end;
    Inc(i);
  end;
...
//    try
      ...
      AddRecipients(EMsg.Recipients, ToField);
      if MailCc <> '' then
        AddRecipients(EMsg.CCList, MailCc);
      if MailBCc <> '' then
        AddRecipients(EMsg.BCCList, MailBCc);
    ...
    end;
{
    ...
    MailItem.Recipients.Add(s);
    if MailCc <> '' then
      MailItem.Recipients.Add(MailCc).Type := 2;
    if MailBCc <> '' then
      MailItem.Recipients.Add(MailBCc).Type := 3;
