...
uses
  ...
  Classes, Graphics, Controls, Forms, Dialogs, Types,
  frxClass, Variants, frxVectorCanvas
  {$IFDEF FPC}
  , LCLType, LCLIntf, LazUTF8, LCLProc, LazHelper, Printers
  {$ELSE}
  , frxUnicodeUtils
  {$ENDIF}
{$IFDEF Delphi10}
...
// data passed by SetXXX calls
    ...
    procedure frxExtTextOut(C: TCanvas; X, Y, add1, i, j: Integer; {$IFDEF FPC} taj: Integer; {$ENDIF}
      CosA, SinA: Extended; s: {$IFDEF FPCUNICODE}String{$ELSE}WideString{$ENDIF};
      AnsiStr: AnsiString);
    ...
    procedure DrawSelectedTextLine(C: TCanvas; const s: {$IFDEF FPCUNICODE}String{$ELSE}WideString{$ENDIF};
      X, Y, DX, LineIndex: Integer; Align: TfrxHAlign; var fh, oldfh: HFont);
...
// Call these methods in the same order
    ...
    procedure SetText(Text: TWideStrings; FirstParaBreak: Boolean = False; TopLineIndex: Integer = 0);
...
// call these methods only after methods listed above
    ...
    function GetLineY: TIntegerDynArray; // Last array element - top of non-existing line "after last"
...
uses
  frxPrinter, SyncObjs {added by Samuray}, frxPlatformServices, frxUtils, Math;
...
begin
  ...
  begin
    CanBreak := False;
    if frxPos(
        frxGetSymbol(s, i), soglasn) <> 0 then
    begin
      CanBreak := True;
      { 2 }
      if (i < frxLength(s)) and
         (frxPos(
           frxGetSymbol(s, i + 1), glasn) <> 0) then
        CanBreak := False;
      { 3 }
      if (i < frxLength(s)) and
         (frxPos(
           frxGetSymbol(s, i + 1), znaks) <> 0) then
        CanBreak := False;
    end;
    if frxPos(
      frxGetSymbol(s, i), glasn) <> 0 then
    begin
      CanBreak := True;
      { 4 }
      if (i < frxLength(s)) and (frxPos(frxGetSymbol(s, i + 1), znaks1) <> 0) then
        CanBreak := False;
      { 6 }
      if (i < frxLength(s) - 2) and
         (frxPos(frxGetSymbol(s, i + 1), soglasn) <> 0) and
         (frxGetSymbol(s, i + 1) = frxGetSymbol(s, i + 2)) and
         (frxPos(frxGetSymbol(s, i + 3), glasn) <> 0) then
        CanBreak := False;
    end;
    if CanBreak then
      Result := Result + Chr(i);
...
var
  ...
  TagsListKeys: TStringList;
  TagsListValues: TStringList;

  procedure Push(el: String; key: String = '');
  begin
    TagsListKeys.Add(el);
    TagsListValues.Add(key);
  end;

  procedure Pop(el: String; key: String = '');
  var
    j, j2: Integer;
    sVal: String;

    procedure DeleteEl(elNum: Integer);
    begin
      TagsListKeys.Delete(elNum);
      TagsListValues.Delete(elNum);
    end;

    function GetFVal(elNum: Integer): String;
    begin
      Result := '<' + TagsListKeys[elNum];
      if (TagsListValues[elNum] <> '') then
        Result := Result + ' ' + TagsListValues[elNum];
      Result := Result + '>';
    end;

  begin
    for j := TagsListKeys.Count - 1 downto 0 do
      if (TagsListKeys[j] = el) then
      begin
        DeleteEl(j);
        for j2 := j - 1 downto 0 do
          if (TagsListKeys[j2] = el) then
          begin
            sVal := GetFVal(j2);
            frxInsert(sVal, s, i + length(el) + length(key) + 3);
            { FPosition used with FPlainText, so we need to correct position by added tags }
            Dec(FPosition, Length(sVal));
            DeleteEl(j2);
            break;
          end;
        break;
...
begin
  ...
  if frxLength(s) = 0 then
    Exit;
  TagsListKeys := TStringList.Create;
  TagsListValues := TStringList.Create;
  while i <= frxLength(s) do
  begin
    b := True;
    // TODO: minimize calls to uppercase
    if FAllowTags then
      if frxGetSymbol(s, i) = '<' then
      begin

        // <b>, <u>, <i> tags
        if (i + 2 <= frxLength(s)) and (frxGetSymbol(s, i + 2) = '>') then
        begin
          cl := frxUpperCase(frxGetSymbol(s, i + 1));
          case cl[1] of
            'B': FStyle := FStyle + [fsBold];
            'I': FStyle := FStyle + [fsItalic];
            'U': FStyle := FStyle + [fsUnderline];
            else
              b := False;
          end;
          if b then
        ...
        else if (i + 1 <= frxLength(s)) and (frxGetSymbol(s, i + 1) = '/') then
        begin
          if (i + 3 <= frxLength(s)) and (frxGetSymbol(s, i + 3) = '>') then
          begin
            cl := frxUpperCase(frxGetSymbol(s, i + 2));
            case cl[1] of
              'B': FStyle := FStyle - [fsBold];
              'I': FStyle := FStyle - [fsItalic];
              'U': FStyle := FStyle - [fsUnderline];
          ...
          else if (frxPos('STRIKE>', frxUpperCase(s)) = i + 2) then
          begin
            Pop('STRIKE');
          ...
          else if (frxPos('NOWRAP>', frxUpperCase(s)) = i + 2) then
          begin
            Pop('NOWRAP');
            FDontWRAP := False;
            frxDelete(s, i, 9);
          ...
          else if frxPos('FONT>', frxUpperCase(s)) = i + 2 then
          begin
            Pop('FONT');
            FColor := FDefColor;
            frxDelete(s, i, 7);
          ...
          else if (frxPos('SUB>', frxUpperCase(s)) = i + 2) or (frxPos('SUP>', frxUpperCase(s)) = i + 2) then
          begin
            if (frxPos('SUB>', frxUpperCase(s)) = i + 2) then
              Pop('SUB')
            else
              Pop('SUP');
            FSize := FDefSize;
        ...
        // <sub>, <sup> tags
        else if (i + 4 <= frxLength(s)) and (frxGetSymbol(s, i + 4) = '>') then
        begin
          b := False;
          if frxPos('SUB>', frxUpperCase(s)) = i + 1 then
          begin
            Push('SUB');
          ...
          else if frxPos('SUP>', frxUpperCase(s)) = i + 1 then
          begin
            Push('SUP');
            FSize := Round(FDefSize / 1.5);
            FAddY := 0;
        ...
        // <sub>, <sup> tags
        else if (i + 5 <= frxLength(s)) and (frxGetSymbol(s, i + 5) = '>') then
        begin
          b := False;
          if (frxPos('/SUB>', frxUpperCase(s)) = i + 1) or (frxPos('/SUP>', frxUpperCase(s)) = i + 1) then
          begin
            if (frxPos('/SUB>', frxUpperCase(s)) = i + 1) then
              Pop('SUB')
            else
        ...
        else if (i + 7 <= frxLength(s)) and (frxUpperCase(frxGetSymbol(s, i + 1)) = 'N') then
        begin
          if frxPos('NOWRAP>', frxUpperCase(s)) = i + 1 then
          begin
            Push('NOWRAP');
            FDontWRAP := True;
            frxDelete(s, i, 8);
        ...
        // <strike> tag
        else if (i + 1 <= frxLength(s)) and (frxUpperCase(frxGetSymbol(s, i + 1)) = 'S') then
        begin
          if frxPos('STRIKE>', frxUpperCase(s)) = i + 1 then
          begin
            Push('STRIKE');
            FStyle := FStyle + [fsStrikeOut];
            frxDelete(s, i, 8);
  ...
  // <font color = ...> tag
        else if frxPos('FONT COLOR', frxUpperCase(s)) = i + 1 then
        begin
          j := i + 11;
          while (j <= frxLength(s)) and (frxGetSymbol(s, j) <> '=') do
            Inc(j);
          Inc(j);
          while (j <= frxLength(s)) and (frxGetSymbol(s, j) = ' ') do
            Inc(j);
          ...
          if cl <> '' then
          begin
            if (frxLength(cl) > 3) and (cl[1] = '"') and (cl[Length(cl)] = '"') then
              cl := Copy(cl, 2, Length(cl) - 2);

            if (cl[1] = '#') then
            begin
              cl := '$' + Copy(cl, 2, Length(cl) - 1);
              FColor := StrToInt(cl);
...
var
  ...
  procedure BreakArray;
  var
    i, offs, PartSz: Integer;
    Size: TSize;

    function DoGetCharsPosArray(nCount: Integer): Boolean;
    var
      idx: Integer;
    begin
      if i + nCount > n then
      ...
      if DefCharset then
      begin
        Result := GetTextExtentExPointA(Canvas.Handle, PAnsiChar(AnsiStr) + i, nCount, 0, nil,
          @FTempArray[i], Size);
      end
      else
        Result := GetTextExtentExPointW(Canvas.Handle, PWideChar(s) + i, nCount, 0, nil,
          @FTempArray[i], Size);
      if not Result then
      begin
        if PartSz <= MinLineToBreak then
          Result := True // cant build break array for MinLineToBreak, try to decrease MinLineToBreak const
        else
          PartSz := nCount div 2
      end
      else
      begin
        if offs <> 0 then
          for idx := i to i + nCount - 1 do
            FTempArray[idx] :=  FTempArray[idx] + offs;
        Inc(i, nCount);
        Result := i >= n;
        offs := FTempArray[i - 1];
      end;
  ...
  begin
    ...
    PartSz := n;
    offs := 0;
    { for some long text line GetTextExtentExPointW may fail without an error }
    { the solution is split a string to smaller blocks and fix offset array }
    { check task #5082 for more information }
    while not DoGetCharsPosArray(PartSz) do;
//  noone know what is this code was used for
//  probably for compat with win 9x
//  remove it in future
//    i := 0;
//    repeat
//      if FTempArray[i] = 32767 then
...
begin
  ...
  begin
    ...
    if i > 0 then
      Ar[i] := FTempArray[i] - FTempArray[i - 1] + tAdd else
      Ar[i] := FTempArray[i] + tAdd;
    if Tag.Small then
      Ar[i] := Round(Ar[i] / 1.5);
    Inc(Result, Ar[i]);
...
procedure TfrxDrawText.SetText(Text: TWideStrings; FirstParaBreak: Boolean = False; TopLineIndex: Integer = 0);
var
  i, j, n, Width: Integer;
...
var
  ...
  begin
    // get the whole word
    i := CurPos;
    while (i <= LineEnd) and (frxPos(frxGetSymbol(s, i), ' .,-;') = 0) do
      Inc(i);
...
// remove all HTML tags and build the tag list
  ...
  Is2ByteCodepage := False;
  if {$IFNDEF FPC}(Win32Platform <> VER_PLATFORM_WIN32_NT) or{$ENDIF} ((Canvas.Font.Charset <> DEFAULT_CHARSET) and not FUseDefaultCharset) then
  begin
    AnsiStr := _UnicodeToAnsi(s, Canvas.Font.Charset);
    n := Length(AnsiStr);
    Is2ByteCodepage := (n > Length(s))
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FText.Add(frxGetSymbol(s, LineBegin)); // can't wrap 1 symbol, just add it to the new line
    ...
    { Index correction }
    if Is2ByteCodepage and (i + 1 <= n) and (Byte(AnsiStr[i + addI]) > $7F) then
      Inc(addI);
    Inc(i);
  end;
...
procedure TfrxDrawText.frxExtTextOut(C: TCanvas; X, Y, add1, i, j: Integer;{$IFDEF FPC} taj: Integer; {$ENDIF}
  CosA, SinA: Extended; s: {$IFDEF FPCUNICODE}String{$ELSE}WideString{$ENDIF}; AnsiStr: AnsiString);
begin
  if ((C.Font.Charset = DEFAULT_CHARSET) or FUseDefaultCharset)
    ...
    begin
      ...
      ExtTextOutExtra(C, X + Round(add1 * SinA), Y + Round(add1 * CosA),
        FOptions, @FScaledRect, PChar(s) + j, i - j, @FTempArray^[taj]);
      if VC <> nil then
        VC.ExtTextOut(X + Round(add1 * SinA), Y + Round(add1 * CosA),
          FOptions, @FScaledRect, PChar(s) + j, i - j, @FTempArray^[taj], C.Font);
      {$ELSE}
  ...
  if C.Font.Charset = OEM_CHARSET then
    ...
    begin
      {$IFDEF FPC}
      ExtTextOutExtra(C, X + Round(add1 * SinA), Y + Round(add1 * CosA),
        FOptions, @FScaledRect, PChar(AnsiStr) + j, i - j, @FTempArray^[taj]);
      if VC <> nil then
        VC.ExtTextOut(X + Round(add1 * SinA), Y + Round(add1 * CosA),
          FOptions, @FScaledRect, PChar(AnsiStr) + j, i - j, @FTempArray^[taj], C.Font);
      {$ELSE}
    ...
    begin
      {$IFDEF FPC}
      ExtTextOutExtra(C, X + Round(add1 * SinA), Y + Round(add1 * CosA),
        FOptions, @FScaledRect, PChar(AnsiStr) + j, i - j, nil);
      {$ELSE}
      ExtTextOutA(C.Handle, X + Round(add1 * SinA), Y + Round(add1 * CosA),
...
var
  ...
  begin
    ...
    begin
      spaceAr[i] := 0;
      if (frxGetSymbol(s, i + 1) = ' ' ) or (frxGetSymbol(s, i + 1) = #$00A0) then
      begin
        Inc(Result);
        spaceAr[i] := 1;
      end;
...
begin
  ...
  try
    ...
    Sz := FHTMLTags.FillCharSpacingArray(FTempArray, s, C, LineIndex,
      Round(FCharSpacing * FScaleX), False, FUseDefaultCharset) - Round(FCharSpacing * FScaleX);
    C.Font.Style := Style;
    ...
    if not FWysiwyg {$IFDEF LCLGTK2} and not IsPrinter(C) {$ENDIF} then
      neededSize := Sz;
    ...
    if not (FHTMLTags.AllowTags and FRTLReading) then
    begin  //LTR parsing Tags
    ...
    repeat
      ...
      {$IFNDEF FPC}
      frxExtTextOut(C, X, Y, add1, i, j, CosA, SinA, s, AnsiStr);
      {$ELSE}
      frxExtTextOut(C, X, Y, add1, i1, j1, j, CosA, SinA, s, AnsiStr);
      {$ENDIF}
    ...
    repeat
      ...
      {$IFNDEF FPC}
      frxExtTextOut(C, X, Y, add1, j, i, CosA, SinA, s, AnsiStr);
      {$ELSE}
      frxExtTextOut(C, X, Y, add1, j1, i1, i, CosA, SinA, s, AnsiStr);
      {$ENDIF}
...
procedure TfrxDrawText.DrawSelectedTextLine(C: TCanvas; const s: {$IFDEF FPCUNICODE}String{$ELSE}WideString{$ENDIF};
  X, Y, DX, LineIndex: Integer; Align: TfrxHAlign; var fh, oldfh: HFont);
var
...
begin
  ...
  R := Rect(
    FScaledRect.Left + 1,
    Min(FLineY[LineIndex], FScaledRect.Bottom) + 1,
    FScaledRect.Right,
    Min(FLineY[LineIndex] + Round(LineHeight * FScaleY), FScaledRect.Bottom));
  C.FillRect(R);
  C.Brush.Color := OldBrushColor;

  OldFontColor := C.Font.Color;
  C.Font.Color := clHighlightText;
  frxSetBkMode(C, TRANSPARENT);

  DrawTextLine(C, s, X, Y, DX, LineIndex, Align, fh, oldfh);
...
var
  Ar: PIntArray;
  i, j, n, neededSize, extraSize, add1, add3, addY: Integer;
  ratio, prn_ratio: Extended;
  al: TfrxHAlign;
  x, y, par, xRotated, yRotated: Integer;
  Sz, prnSz, LineSz, LineSzLs: Integer;
  Tag: TfrxHTMLTag;
  fh, oldfh: HFont;
  h, PPI, dx, gx: Integer;
  CosA, SinA: Extended;
...
begin
  ...
  try
    ...
    if (FHTMLTags.Count > 0) and (FHTMLTags[0].Count > 0) then
    begin
    ...
    begin
      ...
      neededSize := Round((prnSz * n + FLineSpacing * FScaleY * ratio *
        n) / ratio)
    end
    else
    begin
      ratio := FDefPPI / 96;
      neededSize := Round((prnSz * n + FLineSpacing * ratio * n) / ratio
        * FScaleY);
    end;
    ...
    if IsPrinter(C) then
      begin
        prn_ratio := PPI * FPrintScale / 96;
        LineSzLs := Round((FLineSpacing) * prn_ratio);
      end
    else
      begin
        prn_ratio := FScaleY;
        LineSzLs := Round((FLineSpacing - FScaleY) * prn_ratio);
      end;

    LineSz := Sz + Round(FLineSpacing * prn_ratio);

    if (n < 1) and (UnderlinesTextMode = ulmUnderlinesAll) then
      begin
        for j := 1 to Trunc((FScaledRect.Bottom - FScaledRect.Top) / LineSz) + 1 do
          begin
            addY := j * LineSz - LineSzLs;
            if FScaledRect.Top + addY < FScaledRect.Bottom then
              begin
                C.MoveTo(x, FScaledRect.Top + addY);
                C.LineTo(x + dx, FScaledRect.Top + addY);
              end;
          end;
      end;
    ...
    begin
      ...
      xRotated := x + Round(gx * cos(pi / 180 * FRotation));
      yRotated := y - Round(gx * sin(pi / 180 * FRotation));
      FLineY[i] := yRotated;
      if i = SelectedLine then
        DrawSelectedTextLine(C, FText[i], xRotated, yRotated, dx - gx, i, al, fh, oldfh)
      else
        DrawTextLine(C, FText[i], xRotated, yRotated, dx - gx, i, al, fh, oldfh);
        ...
        begin
          if (UnderlinesTextMode = ulmUnderlinesAll) and (i = 0) and (VAlign <> vaTop) then
            for j := 0 to Trunc((y - FScaledRect.Top) / LineSz) do
              begin
                addY := j * LineSz + LineSzLs;
                if y - addY > FScaledRect.Top then
                  begin
                    C.MoveTo(x, y - addY);
                    C.LineTo(x + dx, y - addY);
                  end;
              end;
          if not ((UnderlinesTextMode = ulmUnderlinesText) and (Trim(FText[i]) = '')) then
            begin
              addY := LineSz - LineSzLs;
              if (y + addY <= FScaledRect.Bottom) and (y + addY > FScaledRect.Top) then
                begin
                  C.MoveTo(x, y + addY);
                  C.LineTo(x + dx, y + addY);
                end;
          ...
          if (UnderlinesTextMode = ulmUnderlinesAll) and (i = n - 1) and (VAlign <> vaBottom) then
            for j := 2 to Trunc((FScaledRect.Bottom - y) / LineSz) do
              begin
                addY := j * LineSz - LineSzLs;
                if y + addY <= FScaledRect.Bottom then
                  begin
                    C.MoveTo(x, y + addY);
                    C.LineTo(x + dx, y + addY);
                  end;
...
begin
  ...
  // deleting all outbounds text
      ...
      Tag := FHTMLTags[vl];
      Result := {$IFDEF FPC}frxCopy{$ELSE}Copy{$ENDIF}(FPlainText, Tag[0].Position, Length(FPlainText) - Tag[0].Position + 1);
      if ParaBreak then
        if (Length(Result) > 0) and (Result[1] = ' ') then
          {$IFDEF FPC}frxDelete{$ELSE}Delete{$ENDIF}(Result, 1, 1);
      {$IFDEF FPC}frxDelete{$ELSE}Delete{$ENDIF}(FPlainText, Tag[0].Position, Length(FPlainText) - Tag[0].Position + 1);
...
function TfrxDrawText.GetLineY: TIntegerDynArray;
begin
  SetLength(Result, Length(FLineY) + 1); // + 1 !
  Move(FLineY[0], Result[0], Length(FLineY) * SizeOf(FLineY[0]));
  // Top of non-existing line "after last"
  Result[High(Result)] := Round(LineHeight * FScaleY) + 1;
  if High(Result) > 0 then
    Result[High(Result)] := Result[High(Result)] + Result[High(Result) - 1];
end;
