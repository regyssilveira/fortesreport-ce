...
uses
  ...
  StdCtrls, Buttons, ComCtrls, ExtCtrls, frxClass, frxDesgn, frxBaseForm
  {$IFDEF FPC}
  , LCLType
  {$ENDIF}
  ;
...
type
  ...
  TfrxStdWizardForm = class(TfrxBaseForm)
    Pages: TPageControl;
    FieldsTab: TTabSheet;
    GroupsTab: TTabSheet;
    LayoutTab: TTabSheet;
...
procedure TfrxStdWizardForm.UpdateFormPPI(aNewPPI: Integer);
begin
  inherited;
  //TODO
end;

procedure TfrxStdWizardForm.UpdateResouces;
begin
  inherited;
  Caption := frxGet(5600);
...
begin
  if (AvailableFieldsLB.Count > 0) then
