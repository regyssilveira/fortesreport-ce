...
begin
  result := DeflateInit_(stream, level, ZLIB_VERSIONS, SizeOf(TZStreamRec));
end;

function InflateInit(var stream: TZStreamRec): Integer;
begin
  result := InflateInit_(stream, ZLIB_VERSIONS, SizeOf(TZStreamRec));
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        {$IFDEF FPC}
        zstream.next_out := PByte(PChar(PtrInt(outBuffer) + zstream.total_out));
        {$ELSE}
