...
{$IFDEF FPC}
  ColorBox,
{$ENDIF}
  frxClass, frxGaugeView, frxBaseForm;

type
  TfrxGaugeEditorForm = class(TfrxBaseForm)
...
{$ENDIF}
  ...
  frxGauge, frxEditFrame, frxDPIAwareInt;
...
procedure TfrxGaugeEditorForm.UpdateResouces;
var
  gk: TGaugeKind;
...
begin
  inherited;
  Translate(Self);
  GaugeKindComboBox.Items.BeginUpdate;
  GaugeKindComboBox.Items.Clear;
  for gk := Low(TGaugeKind) to High(TGaugeKind) do
    GaugeKindComboBox.Items.Add(frxResources.Get('GaugeKind' + IntToStr(Ord(gk))));
  GaugeKindComboBox.Items.EndUpdate;
  PointerKindComboBox.Items.BeginUpdate;
  PointerKindComboBox.Items.Clear;
  ...
  PointerKindComboBox.Items.EndUpdate;
end;
