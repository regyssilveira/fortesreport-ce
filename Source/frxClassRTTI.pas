...
uses
  ...
  frxPreviewPages, frxEngine, frxDMPClass, frxVariables, fs_iformsrtti, frxPictureGraphics,
  {$IFDEF FPC}
  LazHelper,
  {$ENDIF}
  frxUnicodeUtils
...
var
  i: Integer;
  GraphicFormats: TfrxGraphicFormats;
begin
  ...
  begin
    ...
    AddEnum('TfrxHyperlinkKind', 'hkURL, hkAnchor, hkPageNumber, hkDetailReport, hkDetailPage, hkCustom, hkNone');
    AddEnum('TfrxProcessAtMode', 'paDefault, paReportFinished, paReportPageFinished, paPageFinished, paColumnFinished, paDataFinished, paGroupFinished, paCustom');
    AddEnum('TPDFSignatureKind', 'skInvisible, skVisible, skEmpty');
    AddEnum('TfrxDataLinkProcessing', 'dpString, dpExpression');
    AddEnumSet('TfrxDataLinkLoading', 'dltOnGetData, dltOnPreview');

    GraphicFormats := GetGraphicFormats;
    for i := 0 to GraphicFormats.Count - 1 do
    begin
      if GraphicFormats[i].GetGraphicClass <> nil then
        AddClass(GraphicFormats[i].GetGraphicClass, 'TGraphic');
    end;

    AddClass(TfrxDataLink, 'TPersistent');
    AddClass(TfrxHyperlink, 'TPersistent');
    with AddClass(TfrxCustomFill, 'TPersistent') do
    begin
      AddMethod('function AsBrushFill: TfrxBrushFill', CallMethod);
      AddMethod('function AsGradientFill: TfrxGradientFill', CallMethod);
      AddMethod('function AsGlassFill: TfrxGlassFill', CallMethod);
    end;
    ...
    AddClass(TfrxObjectProcessing, 'TPersistent');
    ...
    AddClass(TfrxDigitalSignatureView, 'TfrxPictureView');
    ...
    begin
      ...
      AddMethod('procedure PrepareReport', CallMethod);
      AddMethod('procedure Print', CallMethod);
      ...
      AddProperty('DrillState', 'TStrings', GetProp, nil);
    end;
    ...
    begin
      ...
      AddMethod('procedure ResetDuplicates(const BandName: String = '')', CallMethod);
    end;
    ...
    AddMethod('function COUNT(Band: Variant = 0; Flags: Integer = 0): Variant',
      CallMethod, 'ctAggregate');
    { TABLE aggregates }
    AddMethod('function SUM(Band: TfrxComponent): Variant',
      CallMethod, 'ctAggregate');
    AddMethod('function AVG(Band: TfrxComponent): Variant',
      CallMethod, 'ctAggregate');
    AddMethod('function MIN(Band: TfrxComponent): Variant',
      CallMethod, 'ctAggregate');
    AddMethod('function MAX(Band: TfrxComponent): Variant',
      CallMethod, 'ctAggregate');
  end;
end;
...
begin
  ...
  begin
    ...
    else if MethodName = 'RESETDUPLICATES' then
      TfrxPostProcessor(Instance).ResetDuplicates(Caller.Params[0]);
  end
  else if ClassType = TfrxCustomFill then
  begin
    if MethodName = 'ASBRUSHFILL' then
      Result := frxInteger(TfrxCustomFill(Instance) as TfrxBrushFill)
    else if MethodName = 'ASGRADIENTFILL' then
      Result := frxInteger(TfrxCustomFill(Instance) as TfrxGradientFill)
    else if MethodName = 'ASGLASSFILL' then
      Result := frxInteger(TfrxCustomFill(Instance) as TfrxGlassFill)
  end
...
{$ENDIF}
  ...
  begin
    ...
    begin
      { rebuild itself }
      if (TfrxReport(Instance).Script = Script) and Assigned(TfrxReport(Instance).Preview) then
        TfrxReport(Instance).RefreshActivePreviewedReport
      else
        TfrxReport(Instance).ShowReport
    end
    else if MethodName = 'PREPAREREPORT' then
    begin
      { rebuild itself }
      if (TfrxReport(Instance).Script = Script) and Assigned(TfrxReport(Instance).Preview) then
        TfrxReport(Instance).RefreshActivePreviewedReport
      else
        TfrxReport(Instance).PrepareReport
    end
    else if MethodName = 'PRINT' then
      TfrxReport(Instance).Print
  end
...
begin
  ...
  begin
    ...
    else if PropName = 'DRILLSTATE' then
      Result := frxInteger(TfrxReport(Instance).DrillState)
  end
