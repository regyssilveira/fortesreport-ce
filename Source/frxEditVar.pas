...
uses
  ...
  StdCtrls, ComCtrls, frxClass, ExtCtrls, Buttons,
  frxVariables, frxDataTree, frxBaseForm
  {$IFDEF FPC}
  , LCLType
  {$ELSE}
  , ImgList, ToolWin
...
type
  TfrxVarEditorForm = class(TfrxBaseLoadSavePrefForm)
    VarTree: TTreeView;
    ToolBar: TToolBar;
...
uses frxEditStrings, frxUtils, frxRes, IniFiles, frxBaseFormEditorDataTree
{$IFDEF FPC}
  , ImgList
{$ENDIF};
...
{$ENDIF}
  ...
  FScreenScale := Screen.PixelsPerInch;
...
procedure TfrxVarEditorForm.UpdateFormPPI(aNewPPI: Integer);

  procedure AssignToolbarSize(aToolB: TToolBar; aImgList: TCustomImageList);
  begin
...
begin
  ...
  AssignToolbarSize(Toolbar, Toolbar.Images);
  Caption := frxGet(2900);
...
procedure TfrxVarEditorForm.LoadFormPreferences(PreferencesStorage: TObject; DefPreferencesStorage: TObject);
var
  Ini: TCustomIniFile;
  lName: String;
begin
  ...
  ExprMemo.Height := Ini.ReadInteger(lName, 'Split1Pos', 76);
  Panel.Width := Ini.ReadInteger(lName, 'Split2Pos', 400);
end;
...
procedure TfrxVarEditorForm.SaveFormPreferences(PreferencesStorage: TObject; DefPreferencesStorage: TObject);
var
  Ini: TCustomIniFile;
  lName: String;
  lScale: Single;
begin
  ...
  Ini.WriteInteger('Form5.TfrxVarEditorForm', 'Split1Pos', Round(ExprMemo.Height / lScale));
  Ini.WriteInteger('Form5.TfrxVarEditorForm', 'Split2Pos', Round(Panel.Width / lScale));
end;
