...
begin
  ...
  begin
    ...
    FSubSaveCurY := CurY;
    LSubreport := nil;
    if Assigned(FKeepBand) and (FKeepBand.Parent is TfrxReportPage) and TfrxReportPage(FKeepBand.Parent).IsSubReport then
      LSubreport := TfrxReportPage(FKeepBand.Parent).SubReport;
    if Assigned(LSubreport) then
      PreviewPages.PasteObjects(LSubreport.Left, CurY)
    else
