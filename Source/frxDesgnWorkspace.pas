...
uses
  ...
  Dialogs, ExtCtrls, Buttons, frxClass, frxUnicodeCtrls, frxDPIAwareBaseControls,
  Variants, frxHint
  {$IFDEF FPC}
...
type
  ...
  TfrxDesignerWorkspace = class(TfrxDPIAwarePanel)
  protected
    FSkipPaintWindow: Boolean;
...
//    procedure PrepareShiftTree(Band: TfrxBand);
  ...
  public
    ...
    property Intersections: Boolean read FIntersections write FIntersections;
    property IntersectionsColor: TColor read FIntersectionsColor write FIntersectionsColor;
  ...
  TDesgnHintData = class(TBaseHintData)
  private
    FMemoView: TfrxCustomMemoView;
  public
    constructor Create(vHeaderPrefix, vHeaderText: String; vMultiPaint: Boolean; vMemoView: TfrxCustomMemoView); overload;
    property MemoView: TfrxCustomMemoView read FMemoView;
  end;

  TDesgnHintWindow = class(TBaseHintWindow)
  protected
    procedure SetPlainTextOptions; override;
  end;
...
uses
  frxRes, frxDMPClass, frxUtils{$IFNDEF FPC}, frxCtrls{$ENDIF}, frxInPlaceEditors;
...
begin
  ...
  EventParams.DevicePPI := CurrentPPI;
end;
...
procedure TfrxDesignerWorkspace.SetEnableHint(vEnableHint: boolean);
begin
...
procedure TfrxDesignerWorkspace.FakeHintShow(var Msg: TCMHintShow);
begin
  //do nothing
end;
...
begin
  ...
  begin
    ...
    Msg.HintInfo.HintWindowClass := TDesgnHintWindow;
    Msg.HintInfo.HintData := TDesgnHintData.Create('', Header, False, HMemoView);
    Msg.HintInfo.HideTimeout := MaxInt;
  end;
end;

procedure TfrxDesignerWorkspace.WMFRXHintShow(var Msg: TCMHintShow);
begin
  inherited;
...
procedure TfrxDesignerWorkspace.PaintWindow(DC: HDC);
begin
  if not FSkipPaintWindow then
    Inherited;
end;

type
  THackfrxView = class(TfrxView);
...
//  end;
  ...
  procedure DrawIntersections();
  var
    i, j: Integer;
    FBufObjects: TList;

      function frxIntersectRect(const obj1, obj2: THackfrxView): Boolean;
      begin
        Result := (obj1.FX < obj2.FX1)
              and (obj1.FX1 > obj2.FX)
              and (obj1.FY < obj2.FY1)
              and (obj1.FY1 > obj2.FY);
      end;


      function DoHighlighting(c: TfrxView): TRect;
      begin
        c.Highlighting(FCanvas, IntersectionsColor)
      end;

      function CheckParent(obj1, obj2: TfrxComponent): Boolean;
      begin
        Result := False;
        if (csContainer in obj2.frComponentStyle) or (csObjectsContainer in obj2.frComponentStyle) then
        while (obj1.Parent <> nil) do
        begin
          if not (obj1.Parent is TfrxComponent) then
          break;
          obj1 := obj1.Parent;
  ...
  begin
    FBufObjects := TList.Create;

    for i := 0 to FObjects.Count - 1 do
    begin
      cc := TfrxComponent(FObjects[i]);
      if (cc is TfrxView) and not(csContained in cc.frComponentStyle) then
        FBufObjects.Add(cc);
    end;

    for i := 0 to FBufObjects.Count - 1 do
      for j := 0 to FBufObjects.Count - 1 do
      begin
        if i <> j then
          if frxIntersectRect(THackfrxView(FBufObjects[i]), THackfrxView(FBufObjects[j])) then
          begin
            if CheckParent(TfrxView(FBufObjects[i]), TfrxView(FBufObjects[j])) or CheckParent(TfrxView(FBufObjects[j]), TfrxView(FBufObjects[i])) then
              continue;
            DoHighlighting(TfrxView(FBufObjects[i]));
            DoHighlighting(TfrxView(FBufObjects[j]));
          end;
      end;
    FBufObjects.Free;
  end;
...
{$ENDIF}
  ...
  if not (FLastObjectOver is TfrxCustomMemoView) then
...
constructor TDesgnHintData.Create(vHeaderPrefix, vHeaderText: String; vMultiPaint: Boolean; vMemoView: TfrxCustomMemoView);
begin
  inherited Create(vHeaderPrefix, vHeaderText, vMultiPaint);
  FMemoView := vMemoView;
end;

{ TDesgnHintWindow }

procedure TDesgnHintWindow.SetPlainTextOptions;
var
  MemoView: TfrxCustomMemoView;
begin
  MemoView := TDesgnHintData(FBaseHintData).MemoView;
  FDrawTextPlain.SetOptions(MemoView.WordWrap, MemoView.AllowHTMLTags,
    MemoView.RTLReading, MemoView.WordBreak, False, MemoView.Wysiwyg, 0);
end;
