...
{ TRxCustomRichEdit }
  ...
  private
    ...
    { used for call SendMessage from Synch }
    FLResult: LRESULT;
    FWparam: WPARAM;
    FLparam: WPARAM;
    FMsg: UINT;
...
{$ENDIF}
    procedure DoSendSynchMessage;
    procedure DoPerformSynchMessage;
  protected
...
{$IFNDEF DELPHI12}
    function FindTextW(const SearchStr: WideString;
      StartPos, Length: Integer; Options: TRichSearchTypes): Integer;
{$ENDIF}
    ...
    function SendSynchMessage(Msg: UINT; wPar: WPARAM; lPar: LPARAM; DoPerform: Boolean = False): LRESULT;
...
uses Printers, ComStrs, OleConst, OleDlg, OleCtnrs, Registry, Math, frxThreading;
...
{$ELSE}
    ...
    procedure LoadFromStream(Stream: TStream; Encoding: TEncoding); overload; override;
{$ENDIF}
...
begin
  SendSynchMessage(EM_EXGETSEL, 0, LPARAM(@Result));
end;
...
begin
  ...
  SendSynchMessage(EM_EXSETSEL, 0, LPARAM(@CharRange));
  if ScrollCaret then SendSynchMessage(EM_SCROLLCARET, 0, 0);
end;
...
begin
  Result := SendSynchMessage(EM_EXLINEFROMCHAR, 0, CharIndex);
end;

function TRxCustomRichEdit.GetLineIndex(LineNo: Integer): Integer;
begin
  Result := SendSynchMessage(EM_LINEINDEX, LineNo, 0);
end;

function TRxCustomRichEdit.GetLineLength(CharIndex: Integer): Integer;
begin
  Result := SendSynchMessage(EM_LINELENGTH, CharIndex, 0);
end;
